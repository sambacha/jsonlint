...
var options = require("nomnom")
  ...
  .options({
    file: {
      list: true,
      position: 0,
      help: "files or directories to parse; otherwise uses stdin"
    },
    version: {
      flag : true,
      string: '-v, --version',
    ...
    extension: {
      list: true,
      string: '-E ext, --extension ext',
      "default": ['json', 'JSON'],
      help: "file extensions to process for directory walk"
    },
    inplace : {
      flag : true,
      string: '-i, --in-place',
    ...
    comments : {
      flag : true,
      string: '-C, --comments',
      help : 'recognize and ignore JavaScript-style comments'
    },
    validate : {
        string: '-V, --validate',
...
function processSources(src, checkExtension) {
  var extensions = options.extension.map(function (extension) {
    return '.' + extension;
  });
  var srcStat
  try {
...
function main (args) {
  ...
  if (options.file) {
    ...
    for (var i = 0; i < options.file.length; i++) {
      processSources(options.file[i], false)
    }
